...
export default class RegistrationForm {
  ...
  phoneImmediately() {
    if (this.phone.value.length > 13) {
      this.showValidationError(this.phone, "phone name cannot exceed 13 digits with code.")
    }
  ...
  phoneAfterDelay() {
    if (this.phone.value.length < 10) {
      this.showValidationError(this.phone, "Please enter valid phone number.")
    }
  }
  ...
  rePasswordHandler() {
    ...
    this.rePasswordImmediately()
    clearTimeout(this.rePassword.timer)
    this.rePassword.timer = setTimeout(() => this.rePasswordAfterDelay(), 4000)
  }
  passwordImmediately() {
  ...
  rePasswordAfterDelay() {
    if (this.password.value!=this.rePassword.value) {
      this.showValidationError(this.rePassword, "Confirmation password is not matching!!.")
    }
  }
